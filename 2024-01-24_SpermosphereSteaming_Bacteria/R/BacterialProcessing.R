...
###### Decontaminate #####
bac.unedited@sam_data$Sample_or_Control <- ifelse(bac.unedited@sam_data$Type == "Negative Control", "Control Sample", "True Sample")
...
chloro.mito.reads <- sum(sample_sums(bac.unedited))-sum(sample_sums(bac_no_chloro))
...
# these 8 OTUs made up 99.5% of the mock composition. These OTUs also match the 8 supposed to be in the mock
...
######## Should separate the following into a new diversity script #############
