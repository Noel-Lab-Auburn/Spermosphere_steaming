...
Nbac.occ_abund_tax$Steamed2 <- "NonSteamed"
...
nbphylo.diversity.join$Time.Point <- factor(nbphylo.diversity.join$Time, levels = c("17hrs", "Planting", "V2"))
####### Richness over time; supplemental figure part 1 ######
compare_means(richness ~ Trt, nbphylo.diversity.join, group.by = "Time.Point")
compare_means(richness ~ Time, Nsample.data.bac)
...
##### All Samples; Bray #####
```{r}
bac.bray = phyloseq::distance(BSpermosphere_RDS_CSS, "bray") # create bray-curtis distance matrix
```
...
# bray curtis distance
bac.bray.17 = phyloseq::distance(bac_sperm_17, "bray") # create bray-curtis distance matrix
...
###Dispersion for each Time-Point
```{r}
beta.disp.17 <- betadisper(bac.bray.17, bac_sperm_17@sam_data$Trt)

adonis.output.17
permutest(beta.disp.17)

sperm.17.ord <- phyloseq::ordinate(bac_sperm_17, "MDS", "bray")
...
# bray curtis distance
bac.bray.V2 = phyloseq::distance(bac_sperm_V2, "bray") # create bray-curtis distance matrix
...
sperm.V2.ord.plot <- ggplot(sperm.V2.ord.vectors2, aes(x = Axis.1, y = Axis.2, fill = Trt, shape = Type)) +
  ...
  theme(legend.position="bottom",
        ...
        plot.title=element_text(size=10, face = "bold"))
sperm.V2.ord.plot
pcoa_bac.bray <- ggpubr::ggarrange(sperm.0.ord.plot,
                               sperm.17.ord.plot,
                               sperm.V2.ord.plot,
                               labels = "auto",
...
```{r}
##### All Samples ########
bac.unifrac = UniFrac(BSpermosphere_RDS_CSS, weighted = T) # create weighted unifrac distance matrix

# PERMANOVA - testing for differences in centroids
set.seed(12325)
adonis2(bac.unifrac~Crop*Time.Point, as(sample_data(BSpermosphere_RDS_CSS), "data.frame"))
...
### 6 hours; Unifrac #####
```{r}
bac_sperm_6 <- BSpermosphere_RDS_CSS %>% 
...
uni.6.vectors$Tube <- rownames(uni.6.vectors)
uni.6.vectors2 <- left_join(data.frame(bac_sperm_6@sam_data), uni.6.vectors,  by = "Tube")
...
uni.12.vectors$Tube <- rownames(uni.12.vectors)
uni.12.vectors2 <- left_join(data.frame(bac_sperm_12@sam_data), uni.12.vectors,  by = "Tube")
...
### 18 hours; Unifrac #####
```{r}
bac_sperm_18 <- BSpermosphere_RDS_CSS %>% 
...
uni.18.vectors$Tube <- rownames(uni.18.vectors)
uni.18.vectors2 <- left_join(data.frame(bac_sperm_18@sam_data), uni.18.vectors,  by = "Tube")
