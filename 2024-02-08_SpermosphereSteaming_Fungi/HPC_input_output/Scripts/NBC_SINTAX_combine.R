...
# Load in otus.fasta
FASTA.fungi <- readDNAStringSet("clustered/otus.fasta", seek.first.rec=TRUE, use.names=TRUE)
...
NBC_taxonomy <- read.csv("taxonomy/NBCtaxa.csv")
head(NBC_taxonomy)
...
SINTAXa <- read.csv("taxonomy/ITS_taxonomy2.csv", header = F, na.strings = "")
